...
interface TelemetryState {
  ...
  playbackData: TelemetryData[];  // All loaded playback points
  playbackIndex: number;           // Current playback position

  setConnected: (connected: boolean) => void;
  setPlaybackFinalState: (data: TelemetryData | null) => void;
  setPlaybackData: (data: TelemetryData[]) => void;
...
export const useTelemetryStore = create<TelemetryState>((set, get) => ({
  ...
  playbackData: [], // All loaded playback data for trajectory display
  playbackIndex: 0, // Current playback index

  setConnected: (connected) => set({ connected }),

  setPlaybackFinalState: (data) => set({ playbackFinalState: data }),
  setPlaybackData: (data) => set({ playbackData: data, playbackIndex: 0 }),
