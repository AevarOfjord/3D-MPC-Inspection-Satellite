...
// Re-export types and geometry from extracted modules
export type {
...
export function useScanProjectEditor({
  ...
  const setScanAxisAligned = (scanId: string, axis: BodyAxis) => {
    const axisVec = resolveBodyAxisVector(axis, referenceAngle);
    updateScanProject((prev) => ({
      ...prev,
      scans: prev.scans.map((scan) =>
        scan.id === scanId
  ...
  const moveProjectScanPlaneHandle = (
    ...
    if (!scan) return;
    const axisVec = resolveBodyAxisVector(scan.axis, referenceAngle);
  ...
  const updateKeyLevelHandlePosition = (
    ...
    const { normal, uAxis, vAxis } = resolveScanFrameAxes(scan.axis, referenceAngle);
