...
The package has recently been restructured on branch `new`:

- **`simulation/`** — `engine.py` (main sim class), `loop.py`, `context.py`, `initialization.py`, `cpp_backend.py`, and telemetry utilities
...
- **`mission/`** — `runtime_loader.py` (parse/compile missions), `state.py` (MissionState), `unified_compiler.py`
- **`config/`** — Pydantic models, constants, physics/timing parameters
- **`physics/`** — `orbital_config.py` (OrbitalConfig), `orbital_dynamics.py`
- **`dashboard/`** — FastAPI app, routes, simulation manager
- **`exceptions.py`** — Top-level `SimulationError` and siblings (not in a subpackage)

Key import paths after restructure:
```python
...
`TRACK`, `RECOVER`, `SETTLE`, `HOLD`, `COMPLETE` — managed by `v6_policy.py` and `control_loop.py`.

### Frontend (`ui/`)

React 19 + Three.js (`@react-three/fiber`) for 3D visualization. Zustand for state management, Vite for bundling, Vitest + Playwright for tests. The FastAPI backend serves a REST + WebSocket API consumed by the UI.

## Pre-existing Test Failures

Two failures exist on `main` and the `new` branch (not regressions):
...
Expected baseline: **132 passed, 1 skipped, 2 failed**.
