...
def test_default_simulation_timing_contract_config() -> None:
    ...
    assert sim.mpc_target_mean_solve_time_ms == 3.0
    assert sim.mpc_hard_max_solve_time_ms == 50.0
...
def test_mpc_timing_contract_metrics_and_breach_count() -> None:
    ...
    monitor.set_mpc_timing_contract(target_mean_ms=3.0, hard_max_ms=50.0, enforce=False)
...
def test_enforced_mpc_timing_contract_signals_failure() -> None:
    ...
    monitor = PerformanceMonitor()
    monitor.set_mpc_timing_contract(target_mean_ms=3.0, hard_max_ms=50.0, enforce=True)
