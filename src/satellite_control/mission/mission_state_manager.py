...
try:
    ...
    ObstacleAvoidanceSpline = None  # type: ignore
...
class MissionStateManager:
    ...
    def __init__(
        ...
        self.path_tracking_completed: bool = False
        ...
        self._path_tracking_traj_cache: Optional[np.ndarray] = None
        self._path_tracking_traj_cache_id: Optional[int] = None

    def _update_path_tracking_phase(self, phase: str) -> None:
        """Update canonical path-tracking phase in mission_state."""
        self.mission_state.path_tracking_phase = phase

    def _get_path_tracking_phase(self) -> str:
        """Get canonical path-tracking phase from mission_state."""
        return self.mission_state.path_tracking_phase or "POSITIONING"
    ...
    def _get_path_tracking_shape_path(self) -> List[Tuple[float, float, float]]:
        """Get path-tracking shape path from mission_state (V4.0.0: required)."""
        return self.mission_state.path_waypoints

    def _get_path_tracking_target_speed(self) -> float:
        """Get path-tracking target speed from mission_state (V4.0.0: required)."""
        return self.mission_state.path_speed

    def _get_path_tracking_closest_point_index(self) -> int:
        """Get path-tracking closest point index from mission_state (V4.0.0: required)."""
        return self.mission_state.path_tracking_closest_point_index

    def _get_path_tracking_path_length(self) -> float:
        """Get path-tracking path length from mission_state (V4.0.0: required)."""
        return self.mission_state.path_length

    def _get_path_tracking_tracking_start_time(self) -> Optional[float]:
        """Get path-tracking start time from mission_state (V4.0.0: required)."""
        return self.mission_state.path_tracking_tracking_start_time

    def _get_path_tracking_positioning_start_time(self) -> Optional[float]:
        """Get path-tracking positioning start time from mission_state (V4.0.0: required)."""
        return self.mission_state.path_tracking_positioning_start_time

    def _set_path_tracking_positioning_start_time(self, value: float) -> None:
        """Set path-tracking positioning start time in mission_state (V4.0.0: no SatelliteConfig)."""
        self.mission_state.path_tracking_positioning_start_time = value

    def _get_path_tracking_current_target_position(self) -> Optional[Tuple[float, float, float]]:
        """Get path-tracking target position from mission_state (V4.0.0: required)."""
        return self.mission_state.path_tracking_current_target_position

    def _set_path_tracking_current_target_position(
        self, value: Optional[Tuple[float, float, float]]
    ) -> None:
        """Set path-tracking target position in mission_state (V4.0.0: no SatelliteConfig)."""
        self.mission_state.path_tracking_current_target_position = value

    def _get_path_tracking_final_position(self) -> Optional[Tuple[float, float, float]]:
        """Get path-tracking final position from mission_state (V4.0.0: required)."""
        return self.mission_state.path_tracking_final_position

    def _set_path_tracking_final_position(
        self, value: Optional[Tuple[float, float, float]]
    ) -> None:
        """Set path-tracking final position in mission_state (V4.0.0: no SatelliteConfig)."""
        self.mission_state.path_tracking_final_position = value

    def _get_path_tracking_stabilization_start_time(self) -> Optional[float]:
        """Get path-tracking stabilization start time from mission_state (V4.0.0: required)."""
        return self.mission_state.path_tracking_stabilization_start_time

    def _set_path_tracking_stabilization_start_time(self, value: Optional[float]) -> None:
        """Set path-tracking stabilization start time in mission_state (V4.0.0: no SatelliteConfig)."""
        self.mission_state.path_tracking_stabilization_start_time = value

    def _get_path_tracking_target_start_distance(self) -> float:
        """Get path-tracking target start distance from mission_state (V4.0.0: required)."""
        return self.mission_state.path_tracking_target_start_distance

    def _set_path_tracking_target_start_distance(self, value: float) -> None:
        """Set path-tracking target start distance in mission_state (V4.0.0: no SatelliteConfig)."""
        self.mission_state.path_tracking_target_start_distance = value

    def _get_path_tracking_has_return(self) -> bool:
        """Get path-tracking return flag from mission_state (V4.0.0: required)."""
        return self.mission_state.path_tracking_has_return
    ...
    def _get_path_tracking_trajectory_array(self) -> Optional[np.ndarray]:
        """Return cached trajectory array if available."""
        traj = self.mission_state.path_tracking_trajectory
        if not traj:
            self._path_tracking_traj_cache = None
            self._path_tracking_traj_cache_id = None
            return None
        traj_id = id(traj)
        if (
            self._path_tracking_traj_cache is None
            or self._path_tracking_traj_cache_id != traj_id
        ):
            self._path_tracking_traj_cache = np.array(traj, dtype=float)
            self._path_tracking_traj_cache_id = traj_id
        return self._path_tracking_traj_cache

    def _sample_path_tracking_trajectory(
        self, elapsed_time: float
    ) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """Sample trajectory position/velocity at elapsed time."""
        traj = self._get_path_tracking_trajectory_array()
        if traj is None or traj.size == 0:
            return None
        dt = float(self.mission_state.path_tracking_trajectory_dt)
        if dt <= 0:
    ...
    def _predict_path_tracking_trajectory(
        self,
        current_time: float,
        dt: float,
        horizon: int,
        ...
        """Helper to predict trajectory for path-following/path-tracking."""
        path = self.mission_state.path_waypoints
        phase = self.mission_state.path_tracking_phase
        start_time = self.mission_state.path_tracking_tracking_start_time
        speed = self.mission_state.path_speed
        path_len = max(self.mission_state.path_length, 1e-9)
        closest_point_idx = self.mission_state.path_tracking_closest_point_index
        ...
        if traj is not None and phase == "TRACKING" and start_time is not None:
            for k in range(horizon + 1):
                ...
                sample = self._sample_path_tracking_trajectory(elapsed)
                if sample is None:
                    trajectory_out[k] = (
    ...
    def get_trajectory(
        ...
        # Path-only mode
        if self.mission_state.path_following_active:
            self._predict_path_tracking_trajectory(
    ...
    def update_target_state(
        ...
        """
        if self.mission_state.path_following_active:
            return self._handle_path_tracking_mode(
    ...
    def _handle_path_tracking_mode(
        self,
        current_position: np.ndarray,
        current_quat: np.ndarray,
        current_time: float,
    ) -> Optional[np.ndarray]:
        """Handle path-tracking mode."""
        ...
        mission_start_time = self.mission_state.path_tracking_mission_start_time

        if mission_start_time is None:
            self.mission_state.path_tracking_mission_start_time = current_time
            min_dist = float("inf")
            closest_idx = 0
            ...
            self.mission_state.path_tracking_closest_point_index = closest_idx
            self.mission_state.path_tracking_current_target_position = closest_point
        ...
        path_waypoints = self.mission_state.path_waypoints
        phase = self._get_path_tracking_phase()

        # Phase 1: POSITIONING
        if phase == "POSITIONING":
            return self._path_tracking_positioning_phase(
                current_position, current_quat, current_time, path_waypoints
            )

        # Phase 2: TRACKING
        elif phase == "TRACKING":
            return self._path_tracking_tracking_phase(
                current_position, current_time, path_waypoints
            )

        # Phase 3: PATH_STABILIZATION (at path waypoints)
        elif phase == "PATH_STABILIZATION":
            return self._path_tracking_stabilization_phase(
                current_position, current_quat, current_time, path_waypoints
            )

        # Phase 4: STABILIZING
        elif phase == "STABILIZING":
            return self._path_tracking_stabilizing_phase(current_time, path_waypoints)

        # Phase 5: RETURNING
        elif phase == "RETURNING":
            return self._path_tracking_returning_phase(
                current_position, current_quat, current_time
            )

        return None

    def _path_tracking_positioning_phase(
        self,
        current_position: np.ndarray,
        current_quat: np.ndarray,
        current_time: float,
        ...
        """Handle path-tracking positioning phase."""
        # Set phase start time on first entry
        if self._get_path_tracking_positioning_start_time() is None:
            self._set_path_tracking_positioning_start_time(current_time)

        target_pos = self._get_path_tracking_current_target_position()
        ...
        target_orientation = get_path_tangent_orientation(
            path, 0.0, self._get_path_tracking_closest_point_index()
        )
        ...
        if pos_error < self.position_tolerance and ang_error < self.angle_tolerance:
            if self.shape_stabilization_start_time is None:
                ...
                self._update_path_tracking_phase("PATH_STABILIZATION")
                self.mission_state.path_tracking_stabilization_start_time = current_time
                ...
                if stabilization_time >= stab_time:
                    self._update_path_tracking_phase("TRACKING")
                    self.mission_state.path_tracking_tracking_start_time = current_time
                    self._set_path_tracking_target_start_distance(0.0)
                    logger.info(" Satellite stable! Starting profile tracking...")
                    speed = self._get_path_tracking_target_speed()
    ...
    def _path_tracking_tracking_phase(
        self,
        current_position: np.ndarray,
        current_time: float,
        path: List[Tuple[float, ...]],
    ) -> Optional[np.ndarray]:
        """Handle path-tracking tracking phase."""
        tracking_start = self._get_path_tracking_tracking_start_time()
        ...
        if traj is not None and traj.size > 0:
            ...
            if elapsed >= total_time:
                ...
                if has_return:
                    ...
                    return None
                self._update_path_tracking_phase("STABILIZING")
                self._set_path_tracking_stabilization_start_time(current_time)
                self._set_path_tracking_final_position(current_path_position)
                logger.info(
            ...
            if sample is None:
                return None
            pos, vel = sample
            self._set_path_tracking_current_target_position(
        ...
        distance_traveled = self._get_path_tracking_target_speed() * tracking_time
        path_len = max(self._get_path_tracking_path_length(), 1e-9)

        if distance_traveled >= path_len:
            # Path complete
            current_path_position, _ = get_position_on_path(
                path, path_len, self._get_path_tracking_closest_point_index()
            )
            ...
            current_path_position, _ = get_position_on_path(
                path, wrapped_s, self._get_path_tracking_closest_point_index()
            )
            if len(current_path_position) < 3:
                current_path_position = (
                    current_path_position[0],
            ...
            self._set_path_tracking_current_target_position(current_path_position)
    ...
    def _path_tracking_stabilization_phase(
        self,
        current_position: np.ndarray,
        current_quat: np.ndarray,
        current_time: float,
        ...
        """Handle path-tracking waypoint stabilization phase (start or end)."""
        # Determine if we're stabilizing at start or end based on which timer is active
        # and whether the final path-tracking position has been set (V4.0.0: use mission_state)
        is_end_stabilization = self.mission_state.path_tracking_final_position is not None

        if is_end_stabilization:
            # Stabilizing at END of path (before returning)
            target_pos = self.mission_state.path_tracking_final_position
            path_s = self.mission_state.path_length
        else:
            # Stabilizing at START of path (before tracking begins)
            target_pos = self.mission_state.path_tracking_current_target_position
            path_s = 0.0
        ...
        target_orientation = get_path_tangent_orientation(
            path, path_s, self.mission_state.path_tracking_closest_point_index
        )
            ...
            if is_end_stabilization:
                ...
                if self.final_waypoint_stabilization_start_time is None:
                    ...
                    if stabilization_time >= stab_time:
                        ...
                        self.mission_state.path_tracking_phase = "RETURNING"
                        return_pos = self.mission_state.path_tracking_return_position
                ...
                if stabilization_time >= stab_time:
                    ...
                    logger.info(
                        f"   Target speed: {self._get_path_tracking_target_speed():.2f} m/s"
                    )
                    self.shape_stabilization_start_time = None
        else:
    ...
    def _path_tracking_stabilizing_phase(
        self, current_time: float, path: List[Tuple[float, ...]]
    ) -> Optional[np.ndarray]:
        """Handle path-tracking final stabilization phase."""
        # V4.0.0: Use mission_state only
        final_pos = self.mission_state.path_tracking_final_position
        if final_pos is None:
            return None

        # Determine target orientation: use return angle if at return position,
        # otherwise use path tangent
        has_return = self.mission_state.path_tracking_has_return
        return_pos = self.mission_state.path_tracking_return_position
        ...
        if at_return_position:
            ...
            target_orientation = self.mission_state.path_tracking_return_angle or (
                0.0,
                0.0,
                0.0,
            )
            ...
            target_orientation = get_path_tangent_orientation(
                path, end_s, self.mission_state.path_tracking_closest_point_index
            )
        ...
        if self._get_path_tracking_stabilization_start_time() is None:
            self._set_path_tracking_stabilization_start_time(current_time)

        stab_start = self._get_path_tracking_stabilization_start_time()
        ...
        if stabilization_time >= stab_time:
            ...
            self.path_tracking_completed = True
            return None  # Signal mission complete

        return target_state

    def _path_tracking_returning_phase(
        self,
        current_position: np.ndarray,
        current_quat: np.ndarray,
        current_time: float,
    ) -> Optional[np.ndarray]:
        """Handle path-tracking return phase (V4.0.0: use mission_state only)."""
        return_pos = self.mission_state.path_tracking_return_position or (0.0, 0.0, 0.0)
        return_pos_arr = np.array(return_pos, dtype=float)
        if return_pos_arr.shape[0] < 3:
            return_pos_arr = np.pad(
                return_pos_arr, (0, 3 - return_pos_arr.shape[0]), "constant"
            )
        return_angle = self.mission_state.path_tracking_return_angle or (0.0, 0.0, 0.0)
        ...
        if pos_error < self.position_tolerance:
            ...
            if ang_error < self.angle_tolerance:
                ...
                if self.return_stabilization_start_time is None:
                    ...
                    self.mission_state.path_tracking_phase = "STABILIZING"
                    self.mission_state.path_tracking_stabilization_start_time = current_time
                    self.mission_state.path_tracking_final_position = return_pos
                    logger.info(
