...
class SimulationStateValidator:
    ...
    def check_position_bounds(self, position: np.ndarray) -> tuple[bool, str | None]:
        """
    ...
    def check_velocity_bounds(self, velocity: np.ndarray) -> tuple[bool, str | None]:
        """
    ...
    def apply_sensor_noise(
        ...
        use_realistic_physics: bool | None = None,
    ) -> np.ndarray:
        """
...
def create_state_validator_from_config(
    ...
    # Get tolerances from controller.configs.dict, app_config, or fallback to constants
