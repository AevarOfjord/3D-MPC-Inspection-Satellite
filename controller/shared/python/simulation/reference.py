...
from controller.shared.python.utils.orientation_utils import quat_wxyz_from_basis
...
def update_path_reference_state(
    ...
    # Compute reference angular velocity by finite-differencing the reference
    # quaternion.  ω_body = 2 · q_conj ⊗ (dq/dt)  (vector part only).
    q_ref = reference_state[3:7]
    q_prev: np.ndarray | None = getattr(sim, "_prev_ref_q", None)
    dt = float(getattr(getattr(sim, "satellite", None), "dt", 0.0))
    if q_prev is not None and dt > 1e-9:
        # Ensure shortest-path sign consistency to avoid sign-flip artefacts.
        if float(np.dot(q_ref, q_prev)) < 0.0:
            q_prev = -q_prev
        dq = (q_ref - q_prev) / dt
        qw = float(q_ref[0])
        qx = float(q_ref[1])
        ...
        dqy = float(dq[2])
        dqz = float(dq[3])
        # Vector part of  q_conj ⊗ dq  (q_conj = [qw, -qx, -qy, -qz])
        wx = qw * dqx - qx * dqw - qy * dqz + qz * dqy
        wy = qw * dqy + qx * dqz - qy * dqw - qz * dqx
        wz = qw * dqz - qx * dqy + qy * dqx - qz * dqw
        reference_state[10:13] = [2.0 * wx, 2.0 * wy, 2.0 * wz]
    sim._prev_ref_q = q_ref.copy()
