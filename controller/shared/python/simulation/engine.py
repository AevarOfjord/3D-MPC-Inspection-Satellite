...
from controller.configs import AppConfig, SimulationConfig
from controller.configs.constants import Constants
from controller.shared.python.runtime.control_loop import update_mpc_control_step
from controller.shared.python.runtime.path_completion import check_path_complete
from controller.shared.python.simulation.artifact_paths import artifact_path
from controller.shared.python.simulation.initialization import SimulationInitializer
from controller.shared.python.simulation.loop import SimulationLoop
from controller.shared.python.simulation.reference import (
    update_path_reference_state as _update_path_reference_impl,
)
from controller.shared.python.simulation.step_logging import (
    log_simulation_step as log_simulation_step_impl,
)
from controller.shared.python.utils.logging_config import setup_logging
...
try:
    ...
    logger.warning(
        "WARNING: Could not import controller.shared.python.visualization.components. "
...
class SatelliteMPCLinearizedSimulation:
    ...
    def __init__(
        ...
        end_angle: tuple[float, float, float] | None = None,
        start_vx: float = 0.0,
        start_vy: float = 0.0,
        start_vz: float = 0.0,
        ...
        self.structured_config: SimulationConfig | None = None  # Alias
    ...
    def _get_mission_state(self) -> Any | None:
        """Safely get mission state from controller.shared.python.simulation.config."""
        if self.simulation_config is None:
            return None
    ...
    def log_physics_step(self):
        ...
        if getattr(self, "physics_logger_helper", None) is None:
            from controller.shared.python.simulation.logger import (
                SimulationLogger,
            )
    ...
    def step(self) -> None:
        ...
        if getattr(self, "loop", None) is None:
            ...
            if getattr(self, "context", None) is None:
                from controller.shared.python.simulation.context import (
                    SimulationContext,
                )
