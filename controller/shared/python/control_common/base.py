...
class Controller(ABC):
    ...
    def get_shared_contract_signature(self) -> str | None:
        """
        Return immutable shared-parameter contract signature when available.

        Profiles should consume the same shared contract; a non-None signature
        allows callers/tests to assert parity across controller implementations.
        """
        contract = getattr(self, "shared_contract", None)
