...
from controller.shared.python.mission.path_assets import (
    list_path_assets,
    load_path_asset,
    save_path_asset,
)
from controller.shared.python.mission.scan_projects import (
...
async def get_model_bounds(path: str):
    ...
    from controller.shared.python.mission.mesh_scan import (
        compute_mesh_bounds,
        load_obj_vertices,
    )
...
def _normalize3(
    value: list[float] | tuple[float, ...], fallback: tuple[float, float, float]
) -> np.ndarray:
    try:
...
def _axis_frame(axis_seed: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    return np.array([1.0, 0.0, 0.0], dtype=float), np.array(
...
async def generate_scan_path(request: _GenerateScanPathRequest):
    ...
    density_scale = float(
        np.clip(float(request.point_density_scale or 1.0), 0.25, 25.0)
    )
