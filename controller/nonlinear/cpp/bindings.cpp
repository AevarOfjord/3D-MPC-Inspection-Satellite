...
using namespace satellite_control::nonlinear;

PYBIND11_MODULE(_cpp_mpc_nonlinear, m) {
    m.doc() = "Nonlinear RTI-SQP MPC backend (CasADi + OSQP)";

    // -----------------------------------------------------------------------
    // MPCV2Params
    // -----------------------------------------------------------------------
    py::class_<MPCV2Params>(m, "MPCV2Params", py::module_local())
        .def(py::init<>())
        // Horizon
    ...
    // ControlResultV2
    // -----------------------------------------------------------------------
    py::class_<ControlResultV2>(m, "ControlResultV2", py::module_local())
        .def(py::init<>())
    ...
    // SQPController
    py::class_<SQPController>(m, "SQPController", py::module_local())
